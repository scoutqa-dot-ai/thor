...
function validateAlertBody(body: unknown): { ok: true; body: Record<string, unknown> } | { ok: false; error: string } {
...
export function formatNetdataAlertText(body: Record<string, unknown>, publicUrl = ""): string {
  ...
  if (family || chart) lines.push(`*Container/chart:* ${[family, chart].filter(Boolean).join(" / ")}`);
...
export async function handleNetdataAlert(
  ...
  if (!env.SLACK_SUPPORT_CHANNEL_ID) {
    return { ok: false, status: 503, body: { ok: false, error: "SLACK_SUPPORT_CHANNEL_ID is not set" } };
  }
