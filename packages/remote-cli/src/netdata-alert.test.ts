...
describe("remote-cli internal Netdata alert endpoint", () => {
  ...
  beforeEach(async () => {
    fetchMock = vi.fn().mockResolvedValue({ json: async () => ({ ok: true, channel: "COPS", ts: "1.2" }) });
  ...
  it("validates payload shape before posting to Slack", async () => {
    const response = await postAlert({ "x-thor-internal-secret": "internal-secret" }, { status: "CRITICAL" });
  ...
  it("returns a bad-gateway response when Slack rejects the post", async () => {
    fetchMock.mockResolvedValueOnce({ json: async () => ({ ok: false, error: "channel_not_found" }) });
    const response = await postAlert();
