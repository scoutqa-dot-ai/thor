...
describe("gateway", () => {
  it("captures fake Slack Web API calls behind e2e helpers", async () => {
    await withServer(
      vi.fn(
        async () => new Response(JSON.stringify({ accepted: true }), { status: 202 }),
      ) as typeof fetch,
      async (baseUrl) => {
        const body = slackEventBody("Ev2e-capture", {
          type: "app_mention",
          user: "U123",
          channel: "C123",
          text: "<@U0BOTEXAMPLE> hello",
          ts: "1710000000.000001",
        });

        const eventResponse = await postSignedSlackEvent(baseUrl, body);
        expect(eventResponse.status).toBe(200);

        const callsResponse = await fetch(`${baseUrl}/internal/e2e/slack-api/calls`, {
          headers: { "x-thor-internal-secret": "internal-test" },
        });
        expect(callsResponse.status).toBe(200);
        const callsBody = (await callsResponse.json()) as { calls: Array<Record<string, unknown>> };
        expect(callsBody.calls).toMatchObject([
          {
            method: "reactions.add",
            payload: { channel: "C123", timestamp: "1710000000.000001", name: "eyes" },
          },
        ]);

        const unauthorized = await fetch(`${baseUrl}/internal/e2e/slack-api/calls`);
        expect(unauthorized.status).toBe(401);

        const resetResponse = await fetch(`${baseUrl}/internal/e2e/slack-api/reset`, {
          method: "POST",
          headers: { "x-thor-internal-secret": "internal-test" },
        });
        expect(resetResponse.status).toBe(200);

        const emptyResponse = await fetch(`${baseUrl}/internal/e2e/slack-api/calls`, {
          headers: { "x-thor-internal-secret": "internal-test" },
        });
        const emptyBody = (await emptyResponse.json()) as { calls: unknown[] };
        expect(emptyBody.calls).toEqual([]);

        const cardResponse = await fetch(`${baseUrl}/internal/e2e/approval-card`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "x-thor-internal-secret": "internal-test",
          },
          body: JSON.stringify({
            channel: "C123",
            ...
            event: {
              ...
              args: {
                projectKey: "THOR",
                issueTypeName: "Task",
                summary: "Gateway helper test",
                description: "Rendered through the real approval card path",
              },
            },
          }),
        });
        ...
        const cardCallsResponse = await fetch(`${baseUrl}/internal/e2e/slack-api/calls`, {
          headers: { "x-thor-internal-secret": "internal-test" },
        });
        const cardCallsBody = (await cardCallsResponse.json()) as {
          calls: Array<{ method: string; payload: { text?: string; blocks?: unknown[] } }>;
        };
        const cardCall = cardCallsBody.calls.find((call) => call.method === "chat.postMessage");
        expect(cardCall?.payload.text).toBe("Create Jira issue: Gateway helper test");
        expect(JSON.stringify(cardCall?.payload.blocks)).toContain("v3:act-jira:atlassian");
      },
      { e2eTestHelpers: true, internalSecret: "internal-test", slackClient: undefined },
    );
  });
