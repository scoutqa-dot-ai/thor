...
describe("gateway", () => {
  ...
  it("coalesces retried GitHub deliveries with the same delivery id and source timestamp", async () => {
    const fetchImpl = vi.fn<typeof fetch>();
  ...
  it("ignores unmapped GitHub repos with repo_not_mapped behavior", async () => {
    const fetchImpl = vi.fn<typeof fetch>();
    mappedRepos = new Set(["test-repo"]);
      ...
      async (baseUrl, _queue, queueDir) => {
        ...
        const response = await fetch(`${baseUrl}/github/webhook`, {
          ...
          headers: {
            "Content-Type": "application/json",
            "X-Hub-Signature-256": signGitHub(body, "github-secret"),
            "X-GitHub-Delivery": "delivery-unmapped",
            "X-GitHub-Event": "pull_request_review_comment",
          },
          body,
        });
  ...
  it("ignores unsupported GitHub event types", async () => {
    ...
    await withServer(
      fetchImpl,
      async (baseUrl, _queue, queueDir) => {
        const body = JSON.stringify({ repository: { full_name: "acme/thor" } });
        const response = await fetch(`${baseUrl}/github/webhook`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
