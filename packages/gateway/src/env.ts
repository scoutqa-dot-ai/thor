...
export function loadGatewayConfig(env: NodeJS.ProcessEnv = process.env): GatewayConfig {
  ...
  return {
    port: loader.legacyInt("PORT", { defaultValue: 3002 }),
    ...
    slackTimestampToleranceSeconds: loader.legacyInt("SLACK_TIMESTAMP_TOLERANCE_SECONDS", {
      defaultValue: 300,
    }),
    ...
    remoteCliPort: loader.legacyInt("REMOTE_CLI_PORT", { defaultValue: 3004 }),
...
export function validateGatewayGitHubEnv(env: NodeJS.ProcessEnv = process.env): {
  ...
  try {
    githubAppBotId = loader.legacyInt("GITHUB_APP_BOT_ID", { min: 1 });
  } catch {
