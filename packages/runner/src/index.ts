...
function renderSlicePage(
  ...
  for (let idx = 0; idx < slice.records.length; idx++) {
    ...
    if (part?.type === "text") {
      ...
      current.rows.push(
        `<li class="row" data-status="completed"><b>assistant text</b> ${escapeHtml(safeSnippet(text, 300))}</li>`,
      );
      continue;
    }
