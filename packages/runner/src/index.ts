...
function renderApplyPatch(part: ViewerToolPart, durationStr: string | undefined): string {
  ...
  return `<li>${hdr}${renderDiffLines(patchText)}</li>`;
}
...
function renderTaskCard(part: ViewerToolPart, durationStr: string | undefined): string {
  ...
  const promptBlock = prompt
    ? `<div class="task-label">prompt</div><pre>${escapeHtml(safeMultilineSnippet(prompt, 4000))}</pre>`
    : "";
  const outputBlock = output
    ? `<div class="task-label">output</div><pre>${escapeHtml(safeMultilineSnippet(output, 4000))}</pre>`
    : "";
...
function renderSlicePage(
  ...
  for (let idx = 0; idx < slice.records.length; idx++) {
    ...
    if (part?.type === "reasoning") {
      ...
      current.rows.push(`<li><b>reasoning</b> ${escapeHtml(safeSnippet(text, 240))}</li>`);
      continue;
    }
    if (part?.type === "step-finish") {
      stepFinishes++;
  ...
  if (steps.length <= 1) {
    ...
    const stepBlocks = steps.map((step, i) => {
      ...
      return `<li class="step"><div class="step-hdr">${escapeHtml(heading)}</div><ol>${step.rows.join("")}</ol></li>`;
    });
