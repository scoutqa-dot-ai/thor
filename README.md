...
- `remote-cli` exposes `POST /exec/*` endpoints for git, gh, Docker troubleshooting, sandbox, scoutqa, langfuse, metabase, MCP tool calls, direct Slack approval-card posting, and approval status/resolution.
...
| Docker           | `remote-cli /exec/docker`                          | Host Docker socket    | `docker ps`, `docker logs`, and `docker stats` only     |
...
| `DOCKER_SOCKET_GID`             | No       | `compose -> remote-cli`     | Host `/var/run/docker.sock` group id for non-root Docker troubleshooting; set with `stat -c %g /var/run/docker.sock` |
...
- Docker troubleshooting uses normal `docker ...` syntax from OpenCode, but the real Docker CLI and `/var/run/docker.sock` mount exist only in `remote-cli`. If the host socket is not root-group accessible, set `DOCKER_SOCKET_GID=$(stat -c %g /var/run/docker.sock)` before starting compose.
...
- `remote-cli` is the only place tool policy is enforced: MCP allow/approve/hidden tiers, command allowlists, Docker's three-command troubleshooting allowlist, GitHub App installation tokens. OpenCode never holds direct upstream credentials or the Docker socket.
