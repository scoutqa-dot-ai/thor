...
- `remote-cli` exposes `POST /exec/*` endpoints for git, gh, sandbox, scoutqa, metabase, MCP tool calls (Atlassian, Grafana, PostHog, Langfuse, Katalon), direct Slack approval-card posting, and approval status/resolution.
...
| Katalon MCP      | `remote-cli /exec/mcp`                             | API key (HTTP Basic)  | Read-only Katalon True Platform test management         |
...
| `KATALON_API_KEY`               | No       | `remote-cli`                | Katalon True Platform MCP API key (HTTP Basic); required with base URL to enable Katalon; profile variants use `_<PROFILE_NAME>` suffixes                 |
| `KATALON_BASE_URL`              | No       | `remote-cli`                | Katalon True Platform MCP base URL (`https://<sub>.katalon.io`); required with API key to enable Katalon; profile variants use `_<PROFILE_NAME>` suffixes |
