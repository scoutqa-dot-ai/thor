...
| `THOR_BEHAVIOR_EVAL_MODEL`            | No       | local behavior evaluator                    | Wire model ID for manual behavioral evals; codex-lb runs otherwise derive it from `build.md`, while other endpoints require an explicit value                                                                         |
| `THOR_BEHAVIOR_EVAL_REPLICATES`       | No       | local behavior evaluator                    | Positive number of times to run each behavioral-eval checkpoint; defaults to `1`                                                                                                                                      |
| `THOR_BEHAVIOR_EVAL_REASONING_EFFORT` | No       | local behavior evaluator                    | Optional reasoning effort sent to the model and recorded in evaluation artifacts                                                                                                                                      |
| `THOR_BEHAVIOR_EVAL_REQUEST_PARAMS`   | No       | local behavior evaluator                    | Optional JSON object merged into each model request; protected request fields cannot be overridden                                                                                                                    |
...
| `PSQL_DATABASES[_<PROFILE>]`          | No       | `remote-cli`                                | JSON bundle of Postgres connection targets keyed by database alias for the read-only `psql` passthrough; profile-suffixed override falls back to the global value. See [`docs/feat/profile.md`](docs/feat/profile.md) |
| `LD_ACCESS_TOKEN`                     | No       | `remote-cli`                                | LaunchDarkly API token for the read-only `ldcli` passthrough; required (with the other `LD_*` vars) to enable LaunchDarkly                                                                                            |
| `LD_BASE_URI`                         | No       | `remote-cli`                                | LaunchDarkly API base URL (e.g. `https://app.launchdarkly.com`)                                                                                                                                                       |
| `LD_PROJECT`                          | No       | `remote-cli`                                | LaunchDarkly project key                                                                                                                                                                                              |
| `LD_ENVIRONMENT`                      | No       | `remote-cli`                                | LaunchDarkly environment key                                                                                                                                                                                          |
