...
- `remote-cli` exposes `POST /exec/*` endpoints for git, gh, sandbox, scoutqa, metabase, aws, MCP tool calls (Atlassian, Grafana, PostHog, Langfuse), direct Slack approval-card posting, and approval status/resolution.
...
| `THOR_AWS_REGION`                   | No          | `compose`, `workspace-shipper` | AWS region used by the AWS overlay; defaults to `us-east-1`                                                                                                                    |
...
| `ATLASSIAN_AUTH`                    | No          | `remote-cli`, `mitmproxy`      | Global Atlassian MCP auth header and mitmproxy default injection; required with cloud ID to enable Atlassian; profile suffixes are MCP-only                                    |
| `ATLASSIAN_CLOUD_ID`                | No          | `remote-cli`                   | Atlassian cloud ID; required with auth to enable Atlassian; injected server-side into outbound MCP calls, hidden from model-facing tool schemas; profile suffixes are MCP-only |
...
- AWS deployment logging and workspace shipping are opt-in via `docker compose -f docker-compose.yml -f docker-compose.aws.yml up --build -d`; the required overlay variables are listed in Deployment Configuration.
