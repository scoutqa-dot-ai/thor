...
| `SLACK_API_BASE_URL`                | No       | `remote-cli`, `gateway`              | Slack Web API base URL for approval cards, controlled Slack posting, and gateway Slack calls; defaults to `https://slack.com/api`                    |
...
Every Slack surface other than a regular public channel is gated by an explicit allowlist under `slack.private_channel_allowlist`. Only `channel_type === "channel"` (a public, non-shared workspace channel) is admitted without a check; private channels, DMs (`im`), and group DMs (`mpim`) must all have their conversation id on the list. List every conversation id that Thor is permitted to act in:
...
The config key name predates a scope broadening — the list now accepts DM (`D…`) and group-DM (`G…`) ids alongside private-channel ids. Public-channel ids in the list are accepted but unnecessary, since public channels skip the check entirely.

When the channel type is not present on the Slack event (common for `app_mention` and `reaction_added` envelopes), the webhook acknowledges immediately and enqueues the event under a `pending:slack-privacy:` correlation key — mirroring the GitHub `pending:branch-resolve:` pattern. The dispatcher then calls `conversations.info` off the ack path to resolve the surface; if the lookup fails, or if the workspace config cannot be loaded, the gate fails closed and the event is dropped with reason `private_channel_not_allowlisted`. The same audit reason fires for non-allowlisted DMs and MPIMs even though the string still says "private channel" — it is retained for log-grep continuity. The `:eyes:` reaction is posted by the dispatcher only after the gate clears, so events that get dropped never receive a visible acknowledgement. Omitting the `slack` key (or leaving the list empty) means every non-public surface is rejected.
