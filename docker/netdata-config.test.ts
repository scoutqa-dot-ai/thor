...
describe("Netdata docker wiring", () => {
  it("declares the Netdata compose service with required mounts, env, and dependency shape", () => {
    ...
    expect(block).toContain("NETDATA_ALERT_WEBHOOK_URL=http://remote-cli:3004/internal/netdata-alert");
    expect(block).toContain("THOR_INTERNAL_SECRET=${THOR_INTERNAL_SECRET:?set THOR_INTERNAL_SECRET}");
    ...
    expect(block).toContain("./docker/netdata/health_alarm_notify.conf:/etc/netdata/health_alarm_notify.conf:ro");
    expect(block).toContain("./docker/netdata/health.d/thor-containers.conf:/etc/netdata/health.d/thor-containers.conf:ro");
