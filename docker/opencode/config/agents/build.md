...
`psql` connects by a configured database alias, not a host: run `psql <alias> -c "select ..."`. Pass only the alias and query/format flags — host, user, and password are supplied for you, so connection flags like `-h`/`-U`/`-d` are rejected. List the databases on an alias's server with `psql <alias> -l`; if you name an unknown alias, the error lists the ones available to you.
...
3. **Plan** — `task(thinker, Role: plan)`. Thinker writes `plan.md` and inserts an Artifacts row.
...
4. **Iterate** — read the README. If the expected Log line or findings file is missing after a hop, retry once with a corrective prompt then escalate. Otherwise re-dispatch `Role: investigate` for follow-up hops; thinker reads prior findings from the run dir instead of being re-briefed. Do not stop at the first plausible explanation. Treat thinker's open leads like "if you want / I can also / next I would check" as internal planning cues — autonomously continue with parallel sub-agents if possible. Stop when plausible alternatives are exhausted, or progress is blocked by missing access/approval.
5. **Report** — keep an evidence ladder when synthesizing the reply: **Confirmed fact** (directly observed in logs/traces/code/tickets/data), **Strong inference** (best explanation fitting multiple confirmed facts), **Open lead** (plausible but unverified). Treat existing thread theories as context, not proof. Name the repo/system, source types, and key file paths/IDs behind the conclusion. Self-audit before posting: fresh? owner identified? source verified?
