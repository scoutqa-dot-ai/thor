...
fallback mrkdwn body. Use Slack mrkdwn style in stdin: `*bold*` not `**bold**`,
`_italic_`, bullets, and code spans/fences as needed. Do not send markdown tables
on stdin; use `--blocks-file` for table output. For multiline replies, prefer a
heredoc or pipe:
