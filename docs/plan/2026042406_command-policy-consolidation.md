...
- help:
  `git help ...`
- read-only:
  `git status ...`, `git log ...`, `git diff ...`, `git show ...`, `git shortlog ...`, `git ls-files ...`, `git show-ref ...`, `git rev-list ...`, `git diff-tree ...`, `git show-branch ...`, `git ls-tree ...`, `git cherry ...`, `git count-objects ...`, `git range-diff ...`, `git check-ref-format ...`, `git check-ignore ...`
- merge base:
...
- implicit `git push`
- `git pull`
- `git config` mutation, `--global`, `--system`, `--file` (read-only forms are supported — see the `config` entry above)
- `git symbolic-ref` (has a ref-mutating form; excluded until a read-only-shape validator exists)
- `git submodule` (read and write subcommands share the name; excluded until split)
- `git --no-pager`
...
### Phase 5 — Re-widen pure read-only git plumbing

**Changes:**

- add back the read-only plumbing/inspection subcommands dropped in Phase 2 that have no mutating flag shape: `rev-list`, `diff-tree`, `show-branch`, `ls-tree`, `cherry`, `count-objects`, `range-diff`, `check-ref-format`, `check-ignore`, plus `help` and the `version` subcommand
- leave excluded the subcommands that are read-only _only_ in some forms: `symbolic-ref` (write form sets/deletes a ref) and `submodule` (write subcommands mutate `.gitmodules`/checkouts)
- update the Git Surface section above; leave `using-git` unchanged — its "unlisted subcommands are still policy-gated, try the invocation and treat a denial as authoritative" framing already covers newly-allowed plumbing without a rewrite
- close stdin on the `/exec/git` invocation so the stdin-reading forms newly reachable through this widening (`rev-list --stdin`, `diff-tree --stdin`, `check-ignore --stdin`) exit immediately instead of hanging the request, matching the existing `/exec/psql` precedent

**Exit criteria:**

- `packages/remote-cli/src/policy.test.ts` covers the newly allowed subcommands
- workspace typecheck passes
...
| 10  | Re-allow git plumbing/inspection subcommands that have no mutating flag shape (`rev-list`, `diff-tree`, `show-branch`, `ls-tree`, `cherry`, `count-objects`, `range-diff`, `check-ref-format`, `check-ignore`, `help`, `version`) | Phase 2's cut was workflow-scoped, not safety-scoped — these commands were dropped only because no concrete workflow needed them yet, not because they're unsafe. They read the object store/refs and cannot mutate the repo under any argument shape, so gating them added audit surface without a security payoff.                                                                                                                                                                                                 | Keep the narrower Phase 2 list and require a concrete workflow before adding each command back |
| 11  | Keep `symbolic-ref` and `submodule` excluded despite being read-only in some invocations                                                                                                                                          | Both have a plain, easy-to-reach mutating form (`symbolic-ref <name> <ref>` sets a ref; `submodule update`/`add` mutates `.gitmodules` and checked-out state) in the same subcommand name as their read form, so a blanket allow would need a per-shape validator, unlike the other Phase 5 additions which are unconditionally read-only                                                                                                                                                                            | Allow them outright, or ship an under-scoped validator without one                             |
| 12  | Close stdin on `/exec/git` rather than deny `--stdin` per-subcommand                                                                                                                                                              | `rev-list`/`diff-tree`/`check-ignore --stdin` would otherwise block forever since `/exec/git` never pipes stdin — the same class of bug already fixed for `/exec/psql`. Closing stdin globally covers every current and future git subcommand that can read from it, instead of enumerating `--stdin`-accepting flags per subcommand.                                                                                                                                                                                | Deny `--stdin` on the three affected subcommands individually                                  |
| 13  | Leave `--output`, `--ext-diff`, `--textconv`, and `--alternate-refs` unrestricted on read passthrough commands                                                                                                                    | These flags were already reachable through `diff`/`show`/`log`, which have been unrestricted passthrough since Phase 2; restricting them only on the Phase 5 additions would be inconsistent without also revisiting Phase 2. `--ext-diff`/`--textconv` require a local `diff.<driver>.command` config entry to do anything, and `git config` mutation is already blocked, so there's no way for an agent to introduce one. `--output` writes to the local filesystem, which the agent already has direct access to. | Add per-flag validators to the Phase 5 additions only                                          |
