...
| 13  | Relax `remote-cli` confinement **in CI only** (`docker-compose.e2e.yml`, wired via `COMPOSE_FILE`): `apparmor=unconfined` + `cap_add: SYS_ADMIN`. Production is untouched.                                                    | The GitHub Actions runner is a container-in-container that cannot host rootless bwrap: AppArmor `docker-default` denies the mount step (`Failed to make / slave`), and a nested userns cannot map uids (`setting up uid map: Permission denied`) without `CAP_SYS_ADMIN`. The production deploy target imposes neither (Phase 1 `run.sh` validated bwrap there with the seccomp profile alone), so prod keeps default AppArmor and no added caps. The override exists only to exercise the real spawn end-to-end in CI; it is named `docker-compose.e2e.yml` (not `*.override.yml`) so a plain `docker compose up` never loads it. Trade-off accepted: the CI grafana check runs under a more privileged container than prod, so it proves "the child spawns and lists tools," not "it spawns under prod's exact confinement" (that is what the Phase 1 spike + `run.sh` cover). | Add the relaxations to the committed `docker-compose.yml` (weakens production); probe-and-skip the grafana E2E (loses the real-spawn coverage) |
