...
| 15  | Prune `seccomp-bwrap.json` to the compose deployment target instead of vendoring Docker's full cross-architecture profile                                                                                                     | Docker custom seccomp profiles replace the default profile, so the file must keep a default-deny allowlist rather than shrinking to only the bwrap syscalls or flipping to default-allow. The deployed/local compose target is Linux amd64/arm64 with no `CAP_SYS_ADMIN` or other extra caps on `remote-cli`, so non-target architecture mappings and inactive cap-gated portability rules can be removed while preserving the bwrap checks.                                                                                                                                                                                                                                                                                                                                                                     | Keep every MIPS/S390/RISC-V/PPC and unavailable-capability branch from Docker's portable default; or use a smaller default-allow denylist that weakens remote-cli seccomp                                         |
| 16  | Core E2E warms Grafana Cloud before starting the Compose stack by polling Tempo's `/api/echo` through Grafana's datasource proxy with a bounded retry budget.                                                                 | The failed inventory contained Grafana's built-in tools but omitted every proxied Tempo tool, so Grafana's generic health endpoint is too shallow. Tempo documents `/api/echo` as a query-frontend endpoint, and Grafana exposes datasource proxy calls by UID; polling that path wakes and verifies the dependency that supplies the proxied tools without starting a second MCP stack.                                                                                                                                                                                                                                                                                                                                                                                                                         | Grafana `/api/health` polling (does not prove Tempo readiness); a disposable MCP client and container (more machinery than this CI-only instability needs); periodic keepalive workflow (wasteful and still racy) |
...
- Manual / E2E:
  - core E2E preflight polls Tempo's query frontend through Grafana's datasource proxy
    and starts Compose only after it responds successfully;
