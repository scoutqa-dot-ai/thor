...
| 13  | Relax AppArmor for `remote-cli` **in CI only** (`docker-compose.e2e.yml`, wired via `COMPOSE_FILE`), not in production                                                                                                        | Phase 5 E2E surfaced `bwrap: Failed to make / slave: Permission denied` on the GitHub Actions runner — its AppArmor `docker-default` profile denies the `mount` ops bwrap needs, a layer separate from seccomp. The production deploy target does not impose this (Phase 1 `run.sh` passed with seccomp only), so production keeps both LSM layers; only the e2e override adds `apparmor=unconfined` to remote-cli. The file is named `docker-compose.e2e.yml` (not `*.override.yml`) so it is never auto-loaded by a plain `docker compose up`.                                                                                                                                    | Add `apparmor=unconfined` to the committed `docker-compose.yml` (weakens production); make the grafana E2E assertion skip-not-fail (loses CI coverage) |
