...
**Security model (Decision D-1)**: a single credential gates both MCP approval resolution and policy-bypass exec, so its scope covers both surfaces. The boundary controls are: (a) the secret never leaves the internal Docker network (no public-ingress exposure), (b) least-privilege enforced at the network boundary rather than per-feature secrets that drift over time, (c) operators rotate one secret on schedule rather than many.
...
- Every invocation logged with `bin`, `argc`, `cwd`, `exitCode`, `durationMs`. Args themselves are not logged: the endpoint is internal-only and the caller is trusted, and a hardcoded redaction list would inevitably miss sensitive flags from tools like `psql`/`ssh`/`aws`/custom binaries, so omitting args entirely is the safer default.
...
| #   | Decision                                                                                         | Rationale                                                                                                                                                                                                                                                                                                                                                                                   |
| --- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D-1 | One credential gates all gateway↔remote-cli internal endpoints                                   | Per-feature secrets drift apart, multiply ops burden, and create rotation gaps. One credential on the internal network boundary is least-privilege at the right layer. The credential's scope covers both approvals and exec; network isolation is the boundary control.                                                                                                                    |
