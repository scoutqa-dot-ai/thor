...
| D-3 | `POST /internal/exec` has no bin allowlist                                                           | The endpoint exists *because* the policy allowlist is too narrow for legitimate service-to-service maintenance. An allowlist on the bypass path defeats the point. The credential + network boundary is the security control.                                                       |
