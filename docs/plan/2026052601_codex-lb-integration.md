...
Backfill plan for shipping `ghcr.io/soju06/codex-lb` as the OpenAI-compatible upstream for opencode, fronted behind Thor's ingress auth gate. The work landed across the commits on `feat/codex-lb` before this plan existed; this file records the contract, decisions, and exit criteria so future sessions can read the rationale.
...
- codex-lb listens on `2455` (API + dashboard) and `1455` (OAuth callback for `auth.openai.com`); both are mapped to `127.0.0.1` on the host.
- The image runs as user `app`; SQLite store lives at `/var/lib/codex-lb/store.db`.
...
- codex-lb's HTTP responses session bridge multiplexes concurrent same-session HTTP calls onto one upstream WebSocket. opencode forks a title-generation call in parallel with the build call under the same session id, and the bridge mis-routed events between them: the title call's downstream stream received `response.created` and `response.completed` but lost every `response.output_text.delta` in between, so session titles were stuck on `New session - <ts>`. Disabling the bridge is the fix.
- opencode does **not** read codex-lb's `/v1/models`. Its built-in `openai` provider builds the model list from its bundled `models.dev` snapshot (the openai provider plugin's `async models()` returns the catalog and filters it; it never issues an HTTP model-list call). `models.dev`'s network endpoint is blocked by mitmproxy, so opencode falls back to the snapshot compiled into the binary. `small_model` pins title generation to `gpt-5.4-mini` and the provider `whitelist` narrows the snapshot's openai models to `gpt-5.4` / `gpt-5.4-mini` / `gpt-5.5`. Verified against the exact pinned `opencode-ai@1.17.9` Linux arm64 and x64 binaries: the embedded snapshot's `openai` provider contains `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.4-pro`, `gpt-5.4-nano`, `gpt-5.5`, `gpt-5.5-instant`, and `gpt-5.5-pro`. (Earlier plan text asserting opencode discovers models from codex-lb's `/v1/models` was incorrect — these models come from the bundled snapshot, not the proxy.)
- `CODEX_LB_MODEL_REGISTRY_ENABLED=false` (set in `docker-compose.yml`, commit `32cf7122`, not yet deployed) stops codex-lb's model-registry refresh scheduler. The scheduler refreshes the catalog from the **ChatGPT upstream** (reachable via mitmproxy) every 300s, and on each run calls `get_codex_version_cache().get_version()`, which looks up the latest Codex CLI version from GitHub/npm (`api.github.com`, `registry.npmjs.org`) — those hosts are egress-blocked, so the lookup fails and `codex_version.py` logs the failure with `exc_info=True` (full traceback). That GitHub/npm lookup is the _only_ caller of the version cache, so disabling the scheduler removes the noise entirely. Confirmed source-level effects of disabling it:
  - `ModelRefreshScheduler.start()` returns immediately when `enabled` is false, so `ModelRegistry._snapshot` is never populated. `get_models_with_fallback()` then returns `_bootstrap_models`, which is initialised to `{}` and never written anywhere — so codex-lb's `/v1/models` (`_build_models_response`) returns an **empty** `data: []`. (The currently-populated prod `/v1/models` is the result of the scheduler's _successful_ ChatGPT-upstream fetches with the registry still enabled; it is not a static default and does not survive disabling.)
  - The actual request path degrades gracefully: `_filter_accounts_for_model` returns all accounts when `plan_types_for_model` is `None`, `_mapped_model_has_registry_entry` returns `False` so plan-filtering is skipped, and reasoning-effort normalization falls back to a default. So `/v1/responses` still routes.
  - Because opencode never consumes codex-lb's `/v1/models` (previous bullet), the empty list does not affect model selection. Net effect of the flag: GitHub/npm traceback noise gone; codex-lb dashboard / any future `/v1/models` consumer would see an empty catalog; opencode unaffected.
  - **Caveat / latent coupling:** this safety holds only while every whitelisted model (and `small_model`) exists in opencode's bundled `models.dev` snapshot. A future opencode bump that drops `gpt-5.4*`, or a codex-lb-only model slug added to the whitelist, would make opencode depend on a populated `/v1/models` — re-enable the registry (or whitelist GitHub/npm egress so the version lookup stops 403ing) in that case. No narrower knob exists in codex-lb 1.19: `model_registry_client_version` is only the post-failure fallback, not a way to skip the version fetch.
...
| 4   | Pin `small_model: openai/gpt-5.4-mini` with a provider whitelist                   | `models.dev` is blocked by mitmproxy policy, so opencode discovers models from codex-lb's `/v1/models`. Without pinning, opencode picked `gpt-5.4` (full-tier reasoning) for title agent                                                                                                                                                  | Whitelisting `models.dev` in mitmproxy; using `gpt-5.4` for everything                                                                                                                                                                                                                              |
