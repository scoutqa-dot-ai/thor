...
- opencode does **not** read codex-lb's `/v1/models`. Its built-in `openai` provider builds the model list from the `models.dev` snapshot compiled into the binary (it never issues an HTTP model-list call), and `models.dev`'s network endpoint is blocked by mitmproxy anyway. `small_model` pins title generation to `gpt-5.4-mini` and the provider `whitelist` narrows the snapshot's openai models to the three models Thor uses: `gpt-5.4-mini`, `gpt-5.6-sol`, and `gpt-5.6-terra`. All three are present in the pinned `opencode-ai@1.18.4` snapshot. Model selection therefore comes from the bundled snapshot, not the proxy.
  - Verify the exact pinned snapshot offline after every `opencode-ai` bump and before adding a model to `small_model`, an agent, or the provider whitelist:
    ...
    The current pin must print `openai/gpt-5.4-mini`, `openai/gpt-5.6-sol`, and `openai/gpt-5.6-terra`. A new model is supported by the pinned snapshot only when this offline command prints its full provider/model ID; do not infer support from codex-lb's live `/v1/models` response or from strings found in the binary.
...
- `CODEX_LB_MODEL_REGISTRY_ENABLED=false` disables codex-lb's model-registry refresh scheduler, whose periodic Codex-CLI-version lookup against GitHub/npm is egress-blocked and otherwise logs a full traceback every cycle. Disabling it removes that noise; `/v1/responses` still routes (the request path degrades gracefully) and opencode is unaffected since it never consumes codex-lb's `/v1/models`.
  - **Caveat / latent coupling:** this holds only while every whitelisted model exists in opencode's bundled `models.dev` snapshot. A future opencode bump that drops one, or a codex-lb-only model slug added to the whitelist, would make opencode depend on a populated `/v1/models` — re-enable the registry (or whitelist GitHub/npm egress) in that case.
...
**Exit criteria:** opencode sessions reach `gpt-5.6-terra` for build, `gpt-5.6-sol` for coder/thinker, and `gpt-5.4-mini` for title generation; all calls return 200 from codex-lb.
...
| 4   | Pin `small_model: openai/gpt-5.4-mini` with a provider whitelist                   | Without pinning, opencode picked a full-tier reasoning model for the title agent. The whitelist narrows the bundled snapshot to the models Thor actually uses                                                                                                                                                                                                                                                                                                                                                 | Whitelisting `models.dev` in mitmproxy; using the primary build model for titles                                                                                                                                                                                                                                           |
...
| 11  | Use OpenCode 1.18.4's embedded catalog for the GPT-5.6 agent models                | An explicit refresh made `gpt-5.6-sol` and `gpt-5.6-terra` available to 1.17.16, but production egress blocks models.dev and a fetched cache would be mutable external state. The 1.18.4 binary exposes both models offline, preserving the existing reproducible startup design.                                                                                                                                                                                                                             | Re-enable runtime refreshes; seed a network-fetched model cache into the image                                                                                                                                                                                                                                             |
| 12  | Use codex-lb 1.21's bundled GPT-5.6 catalog                                        | The model-registry refresh remains disabled under Thor's egress policy. codex-lb 1.21 includes upstream-verified metadata and request handling for the GPT-5.6 Sol and Terra IDs used by the agents, so its offline fallback matches the configured models.                                                                                                                                                                                                                                                   | Keep 1.19's generic pass-through behavior without GPT-5.6 catalog metadata                                                                                                                                                                                                                                                 |
| 13  | Narrow the codex-lb `/api` match to codex-lb's own dashboard prefixes              | The `/api/*` collision below materialized: a newer OpenCode web UI now calls `/api/session?limit=…&order=desc` (and `/api/event`, `/api/model`, …) from the browser, and the greedy `\|api)` alternative routed those to codex-lb → 404. codex-lb's and OpenCode's `/api/*` top-level segments are disjoint (verified against codex-lb's dashboard HAR and OpenCode's OpenAPI `/doc`), so the regex matches only codex-lb's enumerated prefixes and everything else under `/api/*` falls through to opencode. | Enumerate OpenCode's `/api/*` prefixes in a more-specific `location` instead (OpenCode's UI namespace is the primary, more actively-changing surface, so we keep codex-lb's narrower list as the exception — matching the existing "codex-lb path drift" posture); a `/codex-lb/` prefix or subdomain (same reasons as #7) |
...
- **codex-lb path drift.** The ingress regex enumerates codex-lb's dashboard routes (`/dashboard|accounts|settings` plus the `/api/<prefix>` list). If a future codex-lb version adds a top-level or `/api/*` prefix, the ingress block must learn it or those routes land on opencode's SPA fallback.
- **`/api/*` collision (resolved — decision #13).** opencode's HTTP API and codex-lb's dashboard XHR both live under `/api/*`. This originally worked only because opencode's SPA didn't call `/api/*` from the browser; a newer opencode UI does (`/api/session`, `/api/event`, …), which the greedy `|api)` sent to codex-lb → 404. The regex now matches only codex-lb's disjoint prefixes so opencode's `/api/*` falls through. The two namespaces are disjoint today; if a future codex-lb or opencode version introduces an overlapping `/api/<segment>`, prefix-level routing is no longer sufficient and one service must move under its own path prefix or subdomain.
