...
- opencode does **not** read codex-lb's `/v1/models`. Its built-in `openai` provider builds the model list from its bundled `models.dev` snapshot (the openai provider plugin's `async models()` returns the catalog and filters it; it never issues an HTTP model-list call). `models.dev`'s network endpoint is blocked by mitmproxy, so opencode falls back to the snapshot compiled into the binary. `small_model` pins title generation to `gpt-5.4-mini` and the provider `whitelist` narrows the snapshot's openai models to `gpt-5.4` / `gpt-5.4-mini` (`gpt-5.5` is not in the snapshot and is simply ignored). Verified against the exact pinned `opencode-ai@1.15.10` Linux binaries: the embedded snapshot's `openai` provider contains `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.4-pro`, `gpt-5.4-nano`. (Earlier plan text asserting opencode discovers models from codex-lb's `/v1/models` was incorrect — the `gpt-5.4*` models come from the bundled snapshot, not the proxy.)
