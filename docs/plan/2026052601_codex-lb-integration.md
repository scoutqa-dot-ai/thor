...
- opencode discovers codex-lb's model list via `/v1/models` when `models.dev` is unreachable through mitmproxy; `small_model` pins title generation to `gpt-5.4-mini` and `whitelist` keeps the model picker focused on what codex-lb actually serves.
