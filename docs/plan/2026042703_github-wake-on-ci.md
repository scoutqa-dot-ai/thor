...
terminal (deduped on head_sha so the dozen pings within one second collapse
into one dispatch). This is the planned escape hatch, not a near-term change.
...
- Keep the per-deliveryId pending key so an in-flight branch session is not
  blocked by a `check_suite` we are about to drop; the reroute path collapses
  to "drop or reroute terminal".
