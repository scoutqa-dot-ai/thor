...
| D-12 | Use `express-rate-limit` on `/github/webhook`                                       | CodeQL flagged the async GitHub webhook handler because it performs signature authorization without a route rate limit. The standard Express middleware keeps body parsing behind a cheap per-IP limiter and is already present transitively, so gateway declares it directly and applies it only to this route. |
| D-13 | Hash queued event IDs before building queue filenames                               | CodeQL flagged queue filenames derived from webhook-controlled event IDs. A SHA-256 filename component preserves retry dedupe for identical IDs without allowing raw webhook text to influence filesystem paths.                                                                                                 |
