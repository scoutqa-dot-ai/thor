...
All claims verified against official provider documentation on 2026-03-19. Modal addendum added 2026-03-20.

#### Scorecard

| #   | Dimension (weight)           | Daytona  | Vercel Sandbox | E2B      | Modal    | Cloudflare Sandbox |
| --- | ---------------------------- | -------- | -------------- | -------- | -------- | ------------------ |
| 1   | Thor architecture fit (20%)  | **5**    | **4**          | **4**    | **4**    | **2**              |
| 2   | Identity and lifecycle (15%) | **5**    | **3**          | **5**    | **3**    | **2**              |
| 3   | Source sync ergonomics (15%) | **5**    | **4**          | **3**    | **4**    | **3**              |
| 4   | Live preview URLs (10%)      | **5**    | **4**          | **2**    | **3**    | **3**              |
| 5   | Real-time telemetry (10%)    | **5**    | **4**          | **4**    | **4**    | **3**              |
| 6   | Browser / test support (10%) | **5**    | **3**          | **5**    | **5**    | **3**              |
| 7   | Multi-sandbox support (5%)   | **5**    | **5**          | **4**    | **5**    | **4**              |
| 8   | Pricing / cost fit (15%)     | **5**    | **3**          | **4**    | **3**    | **3**              |
|     | **Weighted total**           | **4.95** | **3.65**       | **3.80** | **3.75** | **2.65**           |
...
- **Modal (4):** Sandboxes support create, reattach via `from_id`, tags (`set_tags`/`get_tags`), listing by tags, detached execution. Strong network controls (`block_network`, `cidr_allowlist`). More platform-shaped than Daytona — requires understanding Modal Workspaces, Environments, Apps, Images, Volumes, and Proxies. No documented sandbox lifecycle webhooks; integration is polling + log streaming.
...
- **Modal (3):** Create, terminate, reattach via `from_id`. Sandbox lifetime 5 minutes default, configurable up to 24 hours. Filesystem and directory snapshots for warm resume (directory snapshots persist 30 days). Memory snapshots are alpha: expire after 7 days, close TCP connections, terminate sandbox on capture, cannot snapshot during `Sandbox.exec`, background processes not properly restored. Resume is snapshot-based recreation, not true pause/resume.
...
- **Modal (4):** `add_local_dir` on Image for efficient upload. `Volume.batch_upload()` for pushing files into mounted Volumes. Filesystem and directory snapshots for warm starts. File access APIs (`open`, `ls`, `mkdir`, `rm`) in alpha. Thor's `base + overlay` model maps cleanly. Volumes are write-once/read-many with last-write-wins — fine for Thor's single-writer model.
...
- **Modal (3):** Public tunnels for TCP and HTTP access. Sandbox Connect Tokens for authenticated HTTP/WebSocket. No first-class signed preview URLs with custom expiry like Daytona — tunnels are the preview mechanism.
...
- **Modal (4):** Streamed stdout/stderr via `Sandbox.exec`. Detached runner reattachment via `from_id`. Polling + log streaming. No webhook-driven lifecycle events found in docs — polling is the safe integration model.
...
- **Modal (5):** Custom images from registries / Dockerfiles / Modal Image definitions. Tunnels for browser-accessible services. First-party examples for Claude Code, OpenCode, Anthropic computer use (Streamlit + noVNC/VNC desktop), and Modal Vibe (app per sandbox over tunnels). Strongest official coding-agent example library.
...
- **Modal (5):** Explicitly designed for "thousands of isolated and secure Sandboxes." Up to 100 containers on Starter, 1,000 on Team. Independent tags per sandbox.
...
| **Modal**      | ~$0.24\*\*                   | ~$72                            | $0 (Starter) / $250 (Team) | **~$42 / ~$222**  |
| **Cloudflare** | ~$0.20\*                     | ~$60\*                          | $5/mo Workers              | **~$65\***        |

\*Cloudflare estimate excludes Worker/DO charges and is not directly comparable.

\*\*Modal: 1 physical core (= 2 vCPU) at $0.142/hr CPU + 4 GiB at $0.097/hr memory. Starter includes $30 compute credit; Team includes $100 compute credit.
...
- **Modal (3):** Starter plan is competitive (~$42 effective) but Thor's scaling target (10–15 concurrent) likely requires Team plan at $250/mo base, making effective cost ~$222. Separate CPU + memory billing dimensions.
...
**Scale-out alternative: Modal (3.75/5)**

Strong coding-agent story with first-party Claude Code and OpenCode examples, excellent parallel sandbox scale ("thousands"), good source sync primitives, strong network controls. Weaker on lifecycle (snapshot-based resume only, 24hr max lifetime, alpha memory snapshots) and more platform-shaped than a pure sandbox backend. Best fit if Thor later needs massive parallel fan-out or wants one provider for sandboxes + adjacent infrastructure (queues, volumes, scheduled jobs).
...
FUTURE:    Thor Runner → Modal SDK → Modal Sandbox (when scale demands it)
```
