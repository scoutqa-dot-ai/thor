...
| 10  | Drop the legacy `slack.thread_id` alias type entirely                                                                    | Greenfield; the channel-qualified `slack.thread` form supersedes it. The legacy type allows a no-channel correlation key to bind anchors, which is incompatible with channel→profile resolution.                                                                                                                                                                                                                                                                             | Tolerate legacy reads for back-compat                                                                                                                                                           |
| 11  | Re-resolve approval routing at click time and reject if ambiguous (reverses an earlier creation-time snapshot)           | Snapshotting at creation defends against config drift but lets an operator approve under a different credential than the one their channel maps to today. Re-resolving + failing on ambiguity is safer than either snapshot or silent re-route.                                                                                                                                                                                                                              | Snapshot routing at creation time                                                                                                                                                               |
...
| 15  | Drop the runtime tool-instructions injection; document MCP via static `build.md` and live `mcp` CLI discovery            | The injected prompt mostly duplicated `build.md`, and its only unique value — per-profile filtering — is delivered more accurately by `mcp` discovery, which hits the live profile resolver instead of a session-start snapshot. Removes `tool-instructions.ts` + its test + the runner injection path.                                                                                                                                                                      | Keep generating per-profile tool instructions; statically enumerate allow/approve names in `build.md` (drift + violates AGENTS.md rule 10)                                                      |
| 16  | Repos are a profile selector, with channel authority and a mixed-profile guard                                           | Repo aliases cover cron/non-Slack sessions and repo-only convenience profiles. For Slack-bound sessions, repo fallback is allowed only when the repo resolves to a repo-only profile; mixed `channels[]` + `repos[]` profiles require the Slack channel to be listed. This keeps team-surface profiles from being borrowed through the agent-writable channel repo override while preserving the cron and repo-only public-channel use cases.                                | Repos as a pure cross-check overlay; flat set-union of all channel+repo signals; allowing any unprofiled Slack channel to borrow mixed team profiles through repo fallback                      |
| 18  | Repos never grant Slack admission; admission stays channel-only                                                          | Admission is the trust boundary and is evaluated in the gateway at trigger time, before any repo context exists. Letting a repo admit a non-public channel would gate the trust boundary on something the agent influences. Repos affect credential resolution only.                                                                                                                                                                                                         | Let a repo's profile membership admit an otherwise-unlisted private/DM/shared channel                                                                                                           |
| 19  | Surface raw `remote-cli` approval-resolve stderr instead of regex-whitelisting safe failure categories                   | When the internal `remote-cli` endpoint returns a valid `ExecResult`, the gateway re-enters the runner with the actual resolver/tool failure text so the agent and operator can see why the approved or rejected action failed.                                                                                                                                                                                                                                              | Treat unknown nonzero resolver results as transport failures; keep expanding stderr regexes until structured safe codes exist                                                                   |
| 20  | Treat the `repo` alias as reverse-index-only anchor metadata, never a forward (value→anchor) routing key                 | A repo is shared by many anchors, so it violates the forward map's one-anchor-per-value assumption: a second anchor stamping the same repo would evict the first's binding, silently breaking repo→profile resolution for the cron sessions repo-stamping exists for. Resolution reads the repo off the reverse `externalKeys` anyway, so the forward entry was dead weight. Gate it behind `ANCHOR_METADATA_ALIAS_TYPES`; check stamp-once with `anchorHasExternalKeyType`. | Keep repo in the forward map (collides across anchors); add a dedicated `repo?: string` reverse field + plumb it through resolution/admin views; split a separate `appendAnchorMetadata` writer |
| 21  | Live MCP list/call requests carry no directory signal; profile routing uses only the session-bound anchor aliases        | `/exec/mcp` is reachable from the agent, and request-body `directory` / wrapper `THOR_OPENCODE_DIRECTORY` values are agent-writable. Credential profile selection must therefore come only from `x-thor-session-id` → anchor → `slack.thread` / trigger-stamped `repo` aliases. The runner still validates trigger directories and stamps the repo alias before the agent receives the session.                                                                              | Keep directory as an MCP authz hint; trust `process.cwd()`; add another agent-provided repo/profile selector                                                                                    |
| 22  | Document profile semantics in `docs/feat/profile.md` as the canonical feature contract                                   | Profile behavior spans Slack admission, repo-stamped credential routing, cron, approval re-resolution, and environment suffixes. Keeping the combination matrix in one feature doc avoids contradictory summaries across docs.                                                                                                                                                                                                                                               | Keep restating the full matrix in every integration doc                                                                                                                                         |
...
**Exit criteria:** MCP tests prove `posthog`/`grafana` can resolve differently per profile. (Approval routing is handled by Phase 6's click-time re-resolution.)
...
Adds repos as a second profile selector alongside channels, so non-Slack/cron sessions and repo-only public-channel workflows can opt into a profile via the repo they operate on. Channels stay authoritative; repos fill in when the channel is silent, or when a Slack-bound session is unprofiled and the repo profile has no `channels[]`. Mixed `channels[]` + `repos[]` profiles require Slack channel membership for Slack-bound sessions. A channel/repo conflict fails fast. Repos never affect Slack admission.
...
- **Precedence resolver.** Generalize the strict resolver so `cp` = strict channel→profile (unchanged, including the multi-channel `{in,out}` = ambiguous rule) and `rp` = strict repo→profile. Fail if either dimension is itself ambiguous, or if `cp` and `rp` are both defined and differ. If `cp` is defined, it wins. If there is no Slack binding, `rp` may select any repo profile. If Slack bindings exist but all map to no profile, `rp` may select only a repo-only profile; a mixed `channels[]` + `repos[]` profile fails closed until the Slack channel is listed in that profile.
- **Live MCP path uses only session-bound aliases.** In `resolveProfileForContext`, resolve from the bound `x-thor-session-id` anchor only; do not accept `directory`, `cwd`, `THOR_OPENCODE_DIRECTORY`, or any other agent-supplied path as a profile input.
- **New `repo` alias for MCP and approval routing.** Add `"repo"` to `ALIAS_TYPES` and stamp it on the anchor at trigger time, then have the strict resolver enumerate `repo` keys alongside `slack.thread`. Cron triggers stamp their target repo and resolve a profile with no Slack binding at all. The Slack-button approval path re-resolves from `action.origin.sessionId` and uses the same alias-only resolver.
  - **Landing point.** Stamp inside `resolveSession` in `packages/runner/src/index.ts`, after `anchorId` is determined. The validated `sessionDirectory` (the required, `isAllowedDirectory`-checked trigger directory also passed to OpenCode) is in scope for **all** trigger types — Slack, cron, GitHub — so no gateway change is needed. Use `extractRepoFromCwd(sessionDirectory)` for the alias value via the existing `appendAlias` primitive.
  - **Stamp once, idempotently.** The alias represents the anchor's repo, so write it only when the anchor has none yet (guard via `reverseLookupAnchor`). Cover both mint paths (correlation-key mint and fresh mint); leave the `requestedSessionId` resume path untouched so a resumed session keeps its original repo even if a later trigger's directory differs.
...
- A public/unlisted channel whose session repo maps to a repo-only profile resolves to that profile; the same channel cannot adopt a mixed `channels[]` + `repos[]` profile through repo fallback.
- A public channel listed in one profile and stamped with a repo in a different profile fails fast (channel/repo conflict).
...
- **Atlassian (and any other integration reachable via direct HTTP) bypasses profile routing through mitmproxy.** Mitmproxy's `${ATLASSIAN_AUTH}` interpolation is global with no notion of session/profile, so an agent that shells out to `curl`/`fetch` against `*.atlassian.net` gets the unsuffixed global credential even when the originating channel belongs to a profile with `ATLASSIAN_AUTH_<PROFILE>` set. The MCP path respects the profile; the egress path does not. Pure-MCP integrations (PostHog, Grafana) have no mitmproxy rule and no gap. Needs a follow-up — likely per-session tagging from runner → OpenCode env → mitmproxy addon, so the addon resolves `ATLASSIAN_AUTH_<PROFILE>` with the same fallback rules as `resolveProxyConfig`. Until that lands, treat Atlassian profile suffixes as best-effort (correct for MCP, leaky for direct egress) and prefer PostHog as the canonical profile example in docs.
