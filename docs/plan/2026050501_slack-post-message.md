...
| D18 | Reject runs of 3+ blank lines on stdin (4+ consecutive newlines) outside fenced code blocks                           | Agents sometimes emit huge whitespace gaps in Slack replies; Slack only collapses to one paragraph break, so runaway blank lines render as dead vertical space. Fenced code blocks are carved out so intentional whitespace inside ` ` survives.                                                                   |
