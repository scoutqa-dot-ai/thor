...
The follow-up is now required. Direct `curl`/`fetch` Slack writes bypass Thor's alias-producing control plane, so outbound Slack threads created from non-Slack sessions no longer bind `slack:thread:{ts}` back to the originating OpenCode session. The gateway and common alias resolver still work once an alias exists, but no current producer observes successful `chat.postMessage` responses and calls the alias writer after `slack-mcp` removal.
...
- `packages/opencode-cli/src/remote-cli.ts` — stdin forwarding support for this endpoint while preserving existing command behavior.
- `packages/remote-cli/src/index.ts` — add `POST /exec/slack-post-message` route.
- `packages/remote-cli/src/slack-post-message.ts` (or similar) plus tests — request validation, Slack API call, response handling, alias registration.
- `docker/mitmproxy/rules.py` plus tests — remove the `/api/chat.postMessage` injection rule while keeping read endpoints, `reactions.add`, upload endpoints, and files URLs.
- `docker/opencode/config/agents/build.md` and `docker/opencode/config/skills/slack/SKILL.md` — replace direct `curl` posting with `slack-post-message`.

The design reuses `SLACK_BOT_TOKEN` (already required by gateway/mitmproxy surfaces); if env usage changes, audit `README.md`, `.env.example`, `docker-compose.yml`, and GitHub workflow env blocks per `AGENTS.md`.
...
| #   | Decision                                                                                                              | Rationale                                                                                                                                                                                                                                                                         |
| --- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | Create a new plan for `slack-post-message` instead of amending `2026042301_refactor-slack-mcp.md`                     | The slack-mcp removal plan explicitly left direct-curl alias repair out of scope. This branch changes the post-removal architecture and needs its own durable plan.                                                                                                               |
...
| D6  | Register aliases in remote-cli after Slack `ok: true` without mutating stdout                                         | remote-cli has the session/call IDs and the existing alias writer. Keeping raw Slack JSON on stdout preserves agent/API expectations.                                                                                                                                             |
| D7  | Treat alias registration failure as a logged side-effect failure after a successful Slack post                        | Retrying a successful post solely because bookkeeping failed can duplicate user-visible Slack messages. Log it, but do not re-post.                                                                                                                                               |
| D8  | Keep channel ID out of alias storage in this branch                                                                   | Existing resolver keys are `slack:thread:{ts}`. Redesigning alias metadata would widen the feature; the CLI requires `--channel` explicitly for now.                                                                                                                              |
...
| D10 | Phase 1 rejects `--format blocks` with a clear validation error                                                       | Strict blocks validation and fallback-text semantics are deferred so the first controlled path ships a narrow stdin-only `mrkdwn` contract.                                                                                                                                       |
| D11 | Do not restrict `slack-post-message` channels by repo/session directory                                               | Outbound posts are controlled by the Thor session boundary and the purpose-built endpoint. Thor may need to post to channels that are not inbound trigger channels for the current repo.                                                                                          |
| D12 | Accept recorded `opencode.subsession` callers                                                                         | Delegated agents receive their child session ID in `x-thor-session-id`; once the runner records the child as `opencode.subsession`, aliases bind to the parent's session anchor like other correlation-producing commands.                                                        |
| D13 | Allow `--blocks-file` to reference absolute temp paths                                                                | Agents commonly create temporary Slack artifacts in a temp directory; blocks files are parsed and validated as a top-level JSON array before posting, so they need not be constrained to the command cwd.                                                                         |
| D14 | Share the temp directory between `opencode` and `remote-cli` with a named Compose volume                              | `slack-post-message` runs in OpenCode but parses block files in remote-cli. A shared temp volume preserves the documented temp-file workflow without host-side setup.                                                                                                             |
| D15 | Do not apply repo/worktree cwd validation to `slack-post-message`                                                     | Slack posting is authorized by the session binding and does not run a repo-scoped command. The shell cwd only resolves relative `--blocks-file` paths, which stay constrained to the temp dir or workspace after realpath.                                                        |
| D16 | Do not validate Slack thread timestamp shape locally                                                                  | Slack owns the `thread_ts` format and may evolve it. Thor requires a non-empty value, forwards it unchanged, and registers the same value for reply aliasing after Slack accepts the post.                                                                                        |
| D17 | Reject CommonMark `**bold**` and markdown table separators on stdin; steer agents to Slack mrkdwn and `--blocks-file` | Agents frequently emitted CommonMark `**bold**` and pipe tables, which Slack renders literally. A narrow stdin guard (with code-span/fence carve-outs) plus doc steering keeps mrkdwn usable while pushing table/block output to `--blocks-file`.                                 |
| D18 | Reject literal `\n` escape sequences on stdin outside code spans/fences                                               | OpenCode sometimes forwards a quoted string with literal `\n` instead of real newlines, which Slack renders verbatim. Blocking the sequence forces a heredoc or `printf` so newlines are real; code spans/fences are carved out so docs mentioning `\n` still post.               |
| D19 | Identify code spans/fences with `markdown-it` instead of a hand-rolled stripper                                       | The hand-rolled stripper mis-handled tilde fences, indented code blocks, and multi-backtick delimiters. `markdown-it` is a lightweight CommonMark tokenizer whose `fence`/`code_block` tokens reliably mark which lines to mask before running the `**`, `\n`, and table regexes. |
