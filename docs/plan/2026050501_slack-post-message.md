...
| D11 | Authorize `slack-post-message` channels from the OpenCode session directory instead of mutable shell cwd | The shell cwd can change during an agent run; repo/channel authorization should use the session directory supplied by the OpenCode plugin so a `cd` does not switch Slack channel policy.                                                            |
