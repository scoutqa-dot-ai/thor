...
| 7   | Treat `ATLASSIAN_AUTH` + `ATLASSIAN_CLOUD_ID` as an all-or-nothing bundle per scope | Matches the langfuse/grafana proxy pattern. A profile must set both `_<PROFILE>` variants together; a partial profile bundle is rejected instead of silently mixing scoped auth with the global cloud ID, and an incomplete global config disables the proxy (returns undefined) rather than throwing. Replaces the earlier behavior where scoped auth fell back to the global cloud ID and a missing cloud ID threw |
