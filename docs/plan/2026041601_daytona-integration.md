...
- [x] **P1 — Sandbox-only subdirectory cwd validation.** `sandbox` may run from a subdirectory that exists only inside the Daytona sandbox after a prior command. The endpoint now validates the lexical worktree path first, resolves the real host worktree root separately, and allows missing subpaths only when they remain inside that resolved root.
...
| 2026-04-27 | Sandbox cwd validation allows missing subpaths under a resolved worktree        | CI exposed a valid flow where a prior sandbox command creates a subdirectory that is not yet present locally. Keep git/gh cwd validation strict, but let sandbox resolve the existing host worktree root and run inside sandbox-only subpaths that stay below that root.                                                               |
