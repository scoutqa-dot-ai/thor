...
| 2026-04-17 | Drop in-memory map; Daytona labels are the source of truth                      | `thor-cwd` + `thor-branch` labels stored on create; `daytona.list({thor-managed, thor-cwd})` finds sandboxes, so remote-cli stays stateless (only `cwdLocks` as ephemeral mutex) and survives restart, at the cost of one extra `daytona.list` per request.                                                                   |
...
| 2026-04-17 | Drop sandbox `<id>` arg from exec/stop; key by cwd                              | Label-based lookup is the single source of truth, so the agent never captures or echoes ids and auto-create/stop/list work without them. Tradeoff: one sandbox per worktree at a time (fine).                                                                                                                                 |
...
| 2026-04-18 | No session-end cleanup; rely on Daytona auto-stop                               | Killing on session end would cut off long-running streaming output; Daytona auto-stop (15 min idle) is sufficient and avoids session lifecycle hooks. Cost of a leaked sandbox is negligible.                                                                                                                                 |
