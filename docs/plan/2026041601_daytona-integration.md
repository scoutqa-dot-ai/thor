...
- [x] **P1 — OpenCode owns sandbox execution timeouts.** Removed generic Thor-side process kill timers from remote-cli exec helpers. Sandbox host-side bundle creation/upload and sandbox command streaming should not add duplicate Thor timeout enforcement; OpenCode remains the timeout/cancellation authority unless Thor defines a separate product/API contract.
