...
| 6   | Extract the generic approval pipeline into `approval-service.ts`, keeping MCP and `gh` only as injected executors | The engine owns the store registry, pending creation, Slack posting, dedup, resolution, and audit logging; executors own only the side effect. Removes the MCP-centric naming leakage. | Leaving the shared lifecycle inside `mcp-handler.ts` with a `GH_APPROVAL_STORE` branch; a second parallel approval coordinator |

### Phase 6 — Extract the approval coordinator (cleanup)

**Changes**

- Add `packages/remote-cli/src/approval-service.ts` exporting `createApprovalService` plus the executor boundary (`ApprovalExecutor`, `ApprovalSystemRejection` for fail-closed at click time, `ApprovalOutcome`, and the shared `ApprovalExecResult`/`ok`/`fail`/`stringify` vocabulary). The engine is a registry: `register(store, executor)` records a store namespace and its executor; only registered stores are consulted and resolved. It owns store persistence, `createPending` (anchor/Slack resolution, Slack post, persistence, pending-approval log, `approval_required` payload), in-flight dedup, `resolve`, `executeApproval` (`approval status`/`list`), and approved/rejected audit logging.
- Add `packages/remote-cli/src/cli-approval.ts`: a CLI-agnostic framework for gating any CLI command behind the approval pipeline. A `CliApprovalDefinition` declares the store namespace, tool id, display name, request-arg builder, resolve-time command recovery, and an optional post-success hook. `gh issue create` is the first registered definition, carrying its display parsing, schema validation, and created-issue alias hook. Adding another approvable CLI later is a new definition entry — no new executor or resolver wiring.
- `mcp-handler.ts` no longer contains CLI-specific behavior or owns the approval lifecycle. `createMcpService(deps, approvalService)` takes the shared engine and registers each MCP proxy store (all `PROXY_NAMES` share one executor that re-resolves profile + reconnects upstream at click time, failing closed). The `McpService` surface is back to MCP-only.
- `index.ts` (`createRemoteCliApp`) is the composition root: it builds the `ApprovalService`, passes it to `createMcpService` (registers MCP stores), then `registerCliApprovals` registers CLI stores. Neither approval surface routes through the MCP service.
- Approval audit logs use the `approval` logger namespace instead of `mcp`; event names and the `/exec/*` JSON contracts are unchanged.
...
- Full `@thor/remote-cli` suite green (incl. the new `cli-approval.test.ts`), plus remote-cli typecheck.

## Implementation risks

- **MCP-centric naming leakage:** `/exec/mcp resolve` and `mcp-handler.ts` are named for MCP but will resolve a GitHub CLI approval. Keep the public contract stable. _(Addressed in Phase 6: the shared lifecycle moves to `approval-service.ts`; the `/exec/mcp resolve` transport name stays unchanged to preserve the gateway contract.)_
