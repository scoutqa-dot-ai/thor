...
- Confluence page creation (`createConfluencePage`) — approve-gated MCP tool; markdown content only
...
| 2026-07-20 | Re-enable `createConfluencePage` as an approval-gated, disclaimer-required write                                                                                                | A concrete page-creation use case now exists. Only markdown `content` is accepted (`contentFormat` / `representation` must be omitted or `markdown`); format is checked when queued and revalidated at approval execution, then the Thor footer is appended before the upstream call. Other Confluence writes remain denied.                                                                     |
...
- `createConfluencePage` accepts only markdown `content`, is approval-gated, and appends the disclaimer immediately before the approved upstream call; unsupported formats fail before an approval is queued and are revalidated at execution.
