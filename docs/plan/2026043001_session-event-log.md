...
5. Find the latest `trigger_start` without a matching later `trigger_end`. Any later `trigger_start` supersedes earlier unclosed starts; those earlier slices are treated as crashed, not active.
6. If an open trigger exists → return `{ sessionId: currentId, triggerId }`.
...
Failure mode: if inference returns anything other than an open trigger (`none`, `depth_exceeded`, `cycle`, `oversized`), the caller fails-fast. The write does not ship without a disclaimer. See "Disclaimer Links" for the per-path details.
...
1. Call `findActiveTrigger(requestSessionId)` using the current request's session id. **Fail-fast** if `none` / `cycle` / `depth_exceeded` / `oversized`: return an error to the LLM ("Cannot create approval: no single active trigger for this session") and persist no action.
...
- remote-cli tests for direct-write disclaimer injection (`gh pr create` flag rewrite, `gh pr comment`, `gh pr review`, and PR review-comment reply via `gh api` raw `body` field); fail-fast on direct write when active trigger is missing/oversized (`gh` exits non-zero, no exec); superseded orphan starts use the latest trigger id; policy denial for `gh pr create --fill`, `gh issue create`, and `gh issue comment`; approve-gated args mutation at create time (Jira ticket/comment); fail-fast approve-create on missing active trigger or missing child parent relation (which returns `none`; no action persisted); per-tool injector throws on missing/wrong-typed field (no action persisted); idempotent approve-resolve replay; **child-session URL correctness** — child-session-originated `gh pr create` and `createJiraIssue` after parent linkage produce URLs whose `<sessionId>` segment is the resolved parent (URL renders 200 in the viewer, not 404).
