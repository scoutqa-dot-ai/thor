...
| 2026-05-16 | Drop `MAX_SESSION_FILE_BYTES` / `SESSION_LOG_MAX_BYTES` entirely; read session logs at any size                                                                                                                             | Session logs carry trigger provenance for disclaimers and approvals — losing access to them silently breaks Thor's audit-trail and write-path correctness, so failing closed at 50 MiB is worse than the alternative. Slow is better than throw. The cap propagated `oversized` through `ActiveTriggerResult`, `AnchorContextResult`, `AnchorSessionState`, `readTriggerSlice`, the viewer's "Slice truncated" page, and the admin diag chip — all removed. Per-record cap (`MAX_RECORD_BYTES`, 4 KiB JSONL line) stays. If a runaway log becomes a real operational risk, the right fix is streaming/bounded buffer reads, not a hard cap; revisit then. (SUPERSEDES the 2026-05-01 "fail-closed oversized handling" decision and inverts the 2026-04-30 "scans the full capped session log" framing.) |
