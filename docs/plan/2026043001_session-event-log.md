...
6. Multiple opens (concurrent `/trigger` calls on different sessions of the same anchor — should not happen given anchor-keyed lock, but verifiable here) → return `{ reason: "ambiguous" }`. Caller fails closed.
...
Failure mode: if inference returns anything other than an open trigger (`none`, `ambiguous`, `oversized`), the caller fails fast. The write does not ship without a disclaimer. See "Disclaimer Links" for the per-path details.
...
2. Call `findActiveTrigger(requestSessionId)`. The helper resolves the request session's anchor (`opencode.session` or `opencode.subsession`), reverse-looks-up every session bound to that anchor, and scans each capped session log for a single open `trigger_start`. **Fail-fast** if `none` / `ambiguous` / `oversized`: the `gh` command exits non-zero with a clear error ("Disclaimer required: no single active trigger for session X — runner state may be broken"). No exec, no artifact.
...
1. Call `findActiveTrigger(requestSessionId)` using the current request's session id. **Fail-fast** if `none` / `ambiguous` / `oversized`: return an error to the LLM ("Cannot create approval: no single active trigger for this session") and persist no action.
...
| 2026-05-03 | `findActiveTrigger` returns `{ anchorId, sessionId, triggerId }`; failure modes collapse to `none` / `ambiguous` / `oversized`                                                                                              | Disclaimer URL is built from `anchorId`; the viewer reads `<sessionId>.jsonl` to assemble the slice. `cycle` and `depth_exceeded` failure modes go away with the anchor flat-membership set. `ambiguous` is the new mode for "two sessions of the same anchor have open triggers at once" — should not happen under anchor-keyed locking, but verifying is cheap.                                                                                                                                                                                                                                        |
