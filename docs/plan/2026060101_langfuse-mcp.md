...
- Resolve the Langfuse upstream in `resolveProxyConfig`: `LANGFUSE_PUBLIC_KEY` +
  ...
  validated https, fail fast if unsafe, no default.
...
bundle resolves, Langfuse is disabled. The resolved host **must be https**, since the key pair
is sent as HTTP Basic; a non-https or malformed value **throws** (fail fast) rather than
connecting and leaking credentials in cleartext.
