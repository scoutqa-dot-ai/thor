...
- Extend deterministic `pnpm test:e2e` to use the single mounted E2E `config.json`, create an actor-bearing trigger context, and verify `/exec/git` and `/exec/gh` attribution through the running services.
...
- `pnpm test:e2e` covers git trailer stamping, email-based duplicate suppression, and gh assignee injection.
