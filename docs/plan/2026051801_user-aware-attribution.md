...
- `/workspace/config/thor.json` is already mounted read-only and loaded by remote-cli via `createConfigLoader` (`packages/common/src/workspace-config.ts:143-164`, `docker-compose.yml:133`).
...
- Duplicate identity protection is not enforced at schema time: `users[]` is internal admin-maintained config, and the lookups deterministically return the first match. If duplicates cause confusion, the attribution log lines surface which record won.
...
- **Prerequisite — the lookup tool must be callable on remote-cli's internal Atlassian MCP path** while staying hidden from the agent-facing proxy policy. If the tool is unavailable, this phase is blocked pending a replacement lookup path.
- **Lookup contract.** `lookupJiraAccountId` takes `{ cloudId, searchString }`. Thor sends `searchString: resolved UserRecord.email`; `cloudId` is injected server-side from the configured Atlassian bundle into all outbound Atlassian MCP calls (see `docs/plan/2026032001_atlassian-mcp.md` Decision 7). Output handling: zero matches → `lookup_no_match`, exactly one match with a usable account id → inject `assignee_account_id`, multiple matches → `lookup_multiple_matches`, tool unavailable/disconnected → `upstream_disconnected`.
...
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Where users live                           | Inside existing `/workspace/config/thor.json` under `users`                                              | Already mounted read-only; loader already validates + hot-reloads.                                                                                                                                                                                                                                                                                                                                     |
...
| Squash-merge survival                      | Not separately addressed                                                                                 | GitHub's squash UI concatenates `Co-authored-by:` trailers into the squash body, so co-author credit usually survives (though the author can strip them). The PR assignee survives squash unconditionally, and the disclaimer footer carries the context link — so a dedicated "Triggered by …" body line was rejected as duplicative.                                                                 |
...
| User identity field validation             | Keep validation minimal for internal config                                                              | `users[]` is admin-maintained config, so fields are trusted beyond required string presence and downstream services reject bad handles. Duplicate-identity rejection was dropped as over-engineered: lookups are deterministic (first match wins), the logs surface which record was selected, and the failure cost is cosmetic mis-attribution.                                                       |
...
| Trigger actor extraction owner             | Gateway extracts and passes `triggerSlackId` / `triggerGithubLogin`                                      | Gateway knows event source and batch order; the runner should not reverse-parse prompt JSON or carry Slack/GitHub payload semantics. Reusing the existing trigger fields keeps schema churn low.                                                                                                                                                                                                       |
| Attribution E2E shape                      | Deterministic service E2E for git commit, `gh pr create`, `gh issue create`, and failed live Jira create | Each step asserts on the injected arg in the logged invocation rather than a successful upstream write: the `gh`/Jira create calls are deliberately made to fail (no PR-write App, a missing label, a fake project key), so the test verifies `--assignee` / `assignee_account_id` injection without creating real artifacts. `lookupJiraAccountId` remains hidden from the agent-facing proxy policy. |
