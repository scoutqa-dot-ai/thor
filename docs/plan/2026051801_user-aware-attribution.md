...
- Extend deterministic `pnpm test:e2e` to use the single mounted E2E `config.json`, create an actor-bearing trigger context, and verify `/exec/git` and real `/exec/gh pr create` attribution through the running services.
...
| Attribution E2E shape                      | Deterministic service E2E for git and live `gh pr create`; mocked handler coverage for Jira | The deterministic path proves remote-cli mutates real `/exec` calls using event-log actor context and the single mounted E2E config. `gh pr create` creates a temporary draft PR and reads assignees back from GitHub, using the workflow actor as the configured assignee. Live Jira issue creation has side effects, and `lookupJiraAccountId` is intentionally hidden from the agent-facing proxy policy, so mocked handler tests cover post-approval lookup and injection.                                |
...
- `pnpm test:e2e` covers git trailer stamping and live `gh pr create` assignee injection; email-based duplicate suppression stays in unit coverage.
