...
- Extend deterministic `pnpm test:e2e` to use the single mounted E2E `config.json`, create an actor-bearing trigger context, and verify `/exec/git`, real `/exec/gh pr create`, and approved Jira create-call attribution through the running services.
...
| Attribution E2E shape                      | Deterministic service E2E for git, live `gh pr create`, and failed live Jira create | The deterministic path proves remote-cli mutates real `/exec` calls using event-log actor context and the single mounted E2E config. `gh pr create` creates a temporary draft PR and reads assignees back from GitHub, using the workflow actor as the configured assignee. Jira assignee E2E approves `createJiraIssue` with a fake project key, verifies `lookupJiraAccountId` and the outgoing `assignee_account_id`, then expects the create call to fail before an issue is created. `lookupJiraAccountId` remains hidden from the agent-facing proxy policy. |
...
- `pnpm test:e2e` covers git trailer stamping, live `gh pr create` assignee injection, and live Jira assignee injection on a create call that fails with a fake project key; email-based duplicate suppression stays in unit coverage.
