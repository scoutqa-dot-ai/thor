...
- An approval whose rendered card body exceeds Slack's block limit uploads the
  **full, untruncated** presentation Markdown as a `.md` file into the same
  Slack thread with a meaningful approval comment. The card's body is replaced
  with a short pointer sentence naming the same action ID carried by the
  file's name and comment — not a truncated, half-cut copy of the content —
  and it does not depend on Slack returning a file URL or on the file message
  landing in any particular position relative to the card.
...
| 9   | Oversize card body renders a fixed pointer sentence naming the action ID, not a `trimForSlack`-truncated copy of the content                                                               | Slack does not guarantee the file message lands above the card, so a truncated preview duplicated (and could conflict with) the file. The action ID is the identifier the file's name and comment already carry, so reusing it — rather than asserting a position or adding a permalink — lets two same-title approvals in one thread pair unambiguously with their own file. | Truncated preview text in the card; a card link to the file's Slack permalink (still two messages, since a file-share reply cannot carry Block Kit buttons, so it added no benefit over the ID) |
...
comment (fail fast), then posts the card, whose oversize body is a fixed
pointer sentence naming the same action ID as the file's name/comment (not a
`trimForSlack`-truncated copy of the content, and no claim about message
order). Upload success depends only on Slack completing the upload; no
permalink is requested or rendered, and a file that reaches Slack is never
deleted. Docs. Tests: oversize → file uploaded with full Markdown +
self-contained comment; upload failure fails the approval; card failure
retains the file; two same-title oversize approvals fired concurrently each
pair with their own file via the shared action ID, not the other's.
...
- An oversize approval uploads a `.md` file with the full presentation Markdown
  ...
  URL and no truncated copy of the content, only a pointer sentence naming the
  same action ID as the file, and a normal approval uploads nothing and is
  byte-for-byte unchanged.
- Two oversize approvals with the same title, posted to the same thread, each
  pair unambiguously with their own uploaded file via that shared action ID —
  proven by a regression test that fires them concurrently.
