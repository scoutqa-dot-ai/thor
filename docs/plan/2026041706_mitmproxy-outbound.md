...
| AD14 | DX | README: full "mitmproxy" section, not a paragraph — match semantics, env interpolation rules, readonly flag, NO_PROXY, restart-vs-hot-reload, troubleshooting table keyed by error code, migration guide from `DATA_ROUTE_*` | Mechanical | P1 | Both DX voices flagged "short paragraph" as insufficient | auto |
