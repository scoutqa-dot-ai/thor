...
| D19 | Do not rely on Daytona session-log streaming for real `opencode run` output in v1                                         | A live March 21, 2026 A/B check showed `executeCommand` returned full `opencode run` output quickly, while the session-log path only surfaced early stderr logs and then stalled without assistant stdout before timeout.                                   |
| D20 | Use file-poll streaming: blocking `executeCommand` with `tee` to a file, poll the file in parallel for incremental output | Live validation confirmed sync exec completes in ~4.5s while all session-based streaming approaches (raw, stdbuf, file-redirect) hang. The file-poll workaround captured incremental NDJSON events at 300ms granularity with correct exit-code propagation. |
...
Recent live validation against Daytona clarified the remaining streaming gap.

| Probe                                                                  | Method                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Outcome                                                                                                                                                                               | Takeaway                                                                                                                              |
| ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Synthetic streaming command                                            | Create a `daytona-medium` sandbox, run a shell command that prints delayed JSON lines and a trailing partial line through the streaming session-log path                                                                                                                                                                                                                                                                                                              | Streaming events arrived incrementally before completion                                                                                                                              | Daytona session-log streaming works in general, and Thor's local line-buffering or NDJSON parsing is not the root problem             |
| Real `opencode run` via streaming exec after explicit `1.2.27` install | In a fresh `daytona-medium` sandbox: remove old binary (`rm -f "$(command -v opencode)"`), install with `sudo /usr/local/share/nvm/current/bin/npm install --global opencode-ai@1.2.27`, verify `opencode --version` equals `1.2.27`, then run `opencode run --format json --print-logs --log-level INFO "Reply with exactly OK and nothing else."` through `executeSessionCommand(...runAsync: true)` and stream logs with `getSessionCommandLogs(..., follow=true)` | Logs streamed correctly for startup and DB migration stages, but no assistant JSON/stdout chunk arrived before a 45-second stream timeout; final session query returned no `exitCode` | Confirms install path on Daytona works, but session-log transport for real `opencode run` still stalls before completion              |
| Real `opencode run` via streaming exec                                 | Create a `daytona-medium` sandbox and run `opencode run --format json --print-logs --log-level INFO "Reply with exactly OK and nothing else."` through the streaming session-log path, first unbounded and then with a 30 second shell timeout                                                                                                                                                                                                                        | Early stderr logs from `opencode` arrived after about 2.3 seconds, but no assistant stdout or JSON arrived before timeout; the bounded run exited 124                                 | The stall is specific to the streaming or session-log path for real `opencode run`, not to sandbox creation or command dispatch       |
| Real `opencode run` via normal exec                                    | Run the same `opencode run` command through the non-streaming `executeCommand` path with no auth file                                                                                                                                                                                                                                                                                                                                                                 | Exit 0 in about 5.4 seconds, with the full buffered output returned at the end                                                                                                        | `opencode run` itself works without auth in the Daytona snapshot; the problem is the streaming transport, not basic command execution |

What we tried:

- Ran a fresh Daytona validation with explicit `opencode-ai@1.2.27` reinstall using full npm path under `sudo` and verified `opencode --version` returns `1.2.27`.
- Confirmed `getSessionCommandLogs` streaming callback receives early log chunks for `opencode run`, including startup and migration messages, but no terminal JSON/plain response before the timeout guard.
- Confirmed baseline session streaming still works for short shell commands (e.g., `sleep 1; echo stream-baseline-ok`) with callback-based completion, so the issue remains specific to real `opencode run`.
- Verified the general Daytona callback path with delayed shell output and partial-line flushing
- Patched `sandboxd` to consume provider callback events incrementally, buffer stdout and stderr by line, parse NDJSON from stdout, and flush trailing partial lines
- Ran real `opencode run` live in Daytona through both the streaming and non-streaming provider branches
- Cleaned up the disposable validation sandboxes after the probes

Follow-up validation (same day) tested five approaches with opencode 1.1.35:

| Approach                             | Streaming?            | JSON events?       | Exit    | Time     |
| ------------------------------------ | --------------------- | ------------------ | ------- | -------- |
| Sync `executeCommand` (baseline)     | No — buffered         | YES (144 lines)    | 0       | 4.5s     |
| Session streaming + `stdbuf -oL`     | stderr only, 0 stdout | NO                 | timeout | 46.6s    |
| Session streaming raw                | stderr only, 0 stdout | NO                 | timeout | 46.8s    |
| Session file-redirect + poll         | Initial INFO only     | NO                 | timeout | 61.6s    |
| **Sync exec with `tee` + file poll** | **YES — incremental** | **YES (3 events)** | **0**   | **3.1s** |

Root cause: opencode's internal event-loop (SSE subscriber, file watchers, bus subscriptions) prevents the process from exiting when run via Daytona `executeSessionCommand` with `runAsync: true`. The sync `executeCommand` path handles process lifecycle correctly.

Proven workaround (D20): run `executeCommand` (blocking) with `tee` to a temp file, poll the file every 300ms via separate `executeCommand` calls. Events arrive in batches at poll granularity, exit code propagates correctly.

Current implication for Phase 3:

- Step 6 is resolved: file-poll streaming is implemented in the Daytona provider's `exec` method (D20)
