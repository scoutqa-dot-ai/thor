...
- **Channel-surface gate** (`isSlackEventGated` in `packages/gateway/src/slack-api.ts` plus `evaluateSlackChannelGate` in `packages/gateway/src/slack-channel-gate.ts`, invoked from `shouldIgnoreForGatedChannel` in `app.ts` and pending privacy resolution in `service.ts`): every accepted Slack event must originate either from a regular public, non-shared channel **or** from a conversation id that appears in `slack.private_channel_allowlist` (`packages/common/src/workspace-config.ts`). The decision is made from `event.channel_type` plus `conversations.info` when the event could be public:
  - `"channel"` → enqueued under `pending:slack-privacy:` so `conversations.info` can confirm it is not Slack Connect/shared before dispatch.
  ...
  - missing → a fresh 60-minute classification cache hit can admit/drop inline; otherwise the webhook acknowledges 200 immediately and enqueues under a `pending:slack-privacy:` correlation key (analogous to `pending:branch-resolve:` in §3.1). `planBatchDispatch` resolves the surface via `conversations.info` (WebClient's default 10s timeout), applies the allowlist, and either reroutes to the real `slack:thread:…` key or drops with `private_channel_not_allowlisted`. Observed `app_mention` envelopes do not include `channel_type` and usually take this path.
    Workspace config load failures, lookup timeouts, and lookup errors **fail closed** - the event is treated as gated and dropped if not on the allowlist, rather than admitted or 5xx'd. Successful channel classifications are cached for 60 minutes. The allowlist key name predates a scope broadening that brought DMs, MPIMs, and Slack Connect/shared channels under the same rule; the audit reason string (`private_channel_not_allowlisted`) is retained for log-grep continuity even though it now fires for any non-public or shared surface. See `docs/plan/2026052201_private-channel-allowlist.md` for the decision log.
