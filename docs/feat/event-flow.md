...
- **Channel-surface gate** (`isSlackEventInPrivateChannelScope` + `isSlackPrivateChannelAllowed` in `packages/gateway/src/slack-api.ts`, invoked from `shouldIgnoreForPrivateChannel` at `app.ts:1005`): every accepted Slack event must originate either from a regular public channel **or** from a conversation id that appears in `slack.private_channel_allowlist` (`packages/common/src/workspace-config.ts`). The decision is made from `event.channel_type`:
  ...
  - missing → fall back to `conversations.info` (1.5s timeout, fail-closed) to resolve `is_private` / `is_im` / `is_mpim`, then apply the same rule. `app_mention` and `reaction_added` envelopes commonly omit `channel_type` and take this path.
  Workspace config load failures, lookup timeouts, and lookup errors **fail closed** — the event is treated as gated and dropped if not on the allowlist, rather than admitted or 5xx'd. The allowlist key name predates a scope broadening that brought DMs and MPIMs under the same rule; the audit reason string (`private_channel_not_allowlisted`) is retained for log-grep continuity even though it now fires for any non-public surface. See `docs/plan/2026052201_private-channel-allowlist.md` for the decision log, including the deferred Phase 4 work that will move the missing-`channel_type` lookup off the synchronous ack path by enqueueing under a `pending:slack-privacy:` correlation key (analogous to `pending:branch-resolve:` in §3.1).
...
- **Side effect**: posts an "eyes" emoji reaction so the user sees Thor received the event before the queue drains. Posted only after the channel-surface gate admits the event so we do not visibly react to events we are about to drop.
